...
#New-ADReplicationSubnet -Name "192.168.57.0/24"
        ...
        get-ADReplicationSiteLink -filter "Name -eq `'$first-$SiteName2`'"
        if (! $?) {
            New-ADReplicationSiteLink -Name "$first-$SiteName2" -SitesIncluded Default-First-Site-Name,$SiteName2
        } else { 
            Write-Host "site link [$first-$SiteName2] created already"
        }
...
function Test-Installation {
    ...
    if($Dc1Session -ne $null) {
        ...
        $rc=Invoke-Command -Session $Session -ArgumentList $ArgumentList -ScriptBlock {
            ...
            Get-DhcpServerv4Failover -ComputerName "$Dc1Name2" -Name "$lannetwork2"
            Write-Host "[$?] guest: Get-DhcpServerv4Failover -ComputerName '$Dc1Name2' -Name '$lannetwork2'"                                                                                         -ForegroundColor $(iif $? "Green" "Red") 

            Get-ADReplicationSubnet -Identity $lannetwork2
            Write-Host "[$?] guest: Get-ADReplicationSubnet -Identity $lannetwork2"  
        }
    }
...
Write-Host "Waiting until $RouterName,$Dc2Name,$Dc2Name is installed with Windows..."
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUYexlJ+qZdYW5zGU1uIwFzfWu
# jc+gghYZMIIDDjCCAfagAwIBAgIQILC/BxlyRYZJ/JpoWdQ86TANBgkqhkiG9w0B
...
# DjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBTq+zCdVXYPIAGIH9s4/jOT
# frX0UzANBgkqhkiG9w0BAQEFAASCAQCNXd6LvUQWGRZFvgnruJiZDL5Ix7BczxIp
# 29KKGvUBv3BrujsW+Rwutrctoxf/B87wyvh/BjVqoUKusqI/nciH2eS8rrz+zrdx
# xgAR8dxWKz3YttZKqNvigvmjxHoGb5AmQVBBTNhy/YQAKuAEJ3/K88kwsYoPYDsT
# MK7yByKkdVCbpW+Vsmlo7+SxY+57CdNFOOiITmKnGrcHbCB8RtdDxfTDd3M4/rsZ
# nhWtqqgrJHOyNHtlTwAZ9bke9WPfEGiBYQvKvVL+B4XMyPlXB5dnfIsZFlS9O36N
# JwXtc4dYXjT/xYIRyhBDGHuAjpZiNs07WbEProLduv0mFIrGDacBoYIDIDCCAxwG
...
# hvcNAQkFMQ8XDTIzMDUxNTA2NDA0MlowLwYJKoZIhvcNAQkEMSIEIHylKRGcqLPO
# lYdib/YVsj48SQleHjCQ5Uk8MJBBMyTHMA0GCSqGSIb3DQEBAQUABIICAHDdxnMt
# gLiUyM4BUCz+PEx6DzNmrZRt0P0J7gQ0fOAK6u1HYbxUFVq7LdtBH01wPGgz1Q6H
# tLIAFhuNwOjzL6thn9dIQ/Ccc2pYFKWIBEwaNPlBawSXrUhFu3E09LhHU4h2d4yY
# b+yV3JmKm3ZZJYjb1GQXQ/hq5aw+lfGVEdfAJO8cmvd+ofrMLICSucfUFqPLLlrx
# Kc60kuGGIqIusL8DwZXa2nFKPJqwWc5HzKD7h4NY3W9OFsknnM+QMYrjiUdZ1fgY
# OLsmIR5FuEecB6HqKeDQRZYIfqfyMBY1ezsIrXBgdRWWRDRXkvDypmCfAWAX5xG0
# aMQKKRJmjZ/ovLnnZ7MSJfPWpGD5Fvj/4aWKQ/OlbhxogL17FW9ZP8FJpNtwkOoi
# FlQ4L4Xl1FW7xSiWC3fRrdEvUHQRvfzFSAXa6DFE7jVAQe6zKRsnimpyeJavMLKI
# F2fJVJj1N4/S2Qe+cQ3rNi/G0cZSqfKrEv6Ao8tb2fD5Qh0HHFXfe2+Of23DV/Qv
# XpafT2+fHvQfQ3sB99eP7F2KN0c4f7sCHD1PB/7djKHUm7II0UudgrVmfeHjFx+d
# bH2O1AnU4/46aas+6xx9pdPT5i898npCvzgGKLW0UkB/pC4bKFAS5WRUAPdHyN5c
# 4C38/+YnunqOYm4uhBk6quZ7TCAyYZVxLQcE
